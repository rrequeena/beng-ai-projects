...
SOLID: Single Responsibility - Ball behavior only
"""
import pygame
...
class Ball(Entity):
    ...
    def handle_paddle_collision(self, paddle_rect: pygame.Rect, is_left_paddle: bool) -> None:
        """
        ...
        if abs(self.velocity_x) > config.BALL_MAX_SPEED:
            self.velocity_x = config.BALL_MAX_SPEED if self.velocity_x > 0 else -config.BALL_MAX_SPEED
